...
async def get_json(url: str, *, params: dict | None = None) -> Any:
    """GET `url` with optional query params, return decoded JSON (dict or list)."""
...
async def post_json(url: str, *, json: dict) -> Any:
    """POST `json` body to `url`, return decoded JSON (dict or list)."""
    async with httpx.AsyncClient(timeout=HTTP_TIMEOUT_SECONDS) as client:
